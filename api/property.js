...
export async function addPropertyWithPhotos(property, photoUrls) {
  try {
    ...
    if (!propertyRow) {
      ...
      const generatedId = await generateUnique8DigitId();
    ...
    if (Array.isArray(photoUrls)) {
      await pool.query('UPDATE public.properties SET images = $1 WHERE id = $2', [JSON.stringify(photoUrls || []), propertyId]);
      photos = photoUrls.filter(Boolean);
    } else {
      const pImgs = await pool.query('SELECT images FROM public.properties WHERE id = $1', [propertyId]);
    ...
    try {
      if (photos && photos.length) {
        ...
        if (!current) {
          try {
            await pool.query('UPDATE public.properties SET image_url = $1 WHERE id = $2', [photos[0], propertyId]);
    ...
    if (didInsertNew) {
      try {
        ...
        try {
          ...
          try {
            await pool.query(`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumtypid = 'notification_category'::regtype AND enumlabel = 'properties') THEN ALTER TYPE notification_category ADD VALUE 'properties'; END IF; END$$;`);
            await pool.query(
              `INSERT INTO public.notifications (category, title, body, target, data, is_read, created_at, updated_at)
               VALUES ($1,$2,$3,$4,$5,$6,now(),now())`,
              ['properties', (p.title || propertyRow.title) ? `New property: ${p.title || propertyRow.title}` : 'New property posted', p.description || null, null, JSON.stringify(notifPayload), false]
    ...
    try{
      if(propertyId){
        const verify = await pool.query('SELECT id, title, created_at FROM public.properties WHERE id = $1', [propertyId]);
